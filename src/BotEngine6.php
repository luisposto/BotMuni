...
// =========================
if ($mode !== 'HUMAN') {
    // Zona horaria configurable para decidir "una vez por día"
    ...
    $lastGreetedAt = (string)($session['last_greeted_at'] ?? '');
    $lastGreetDay = ($lastGreetedAt !== '' ? substr($lastGreetedAt, 0, 10) : '');
    $hasGreetedToday = ($lastGreetDay === $today);
    ...
    // =========================
    // SALUDOS (ANTES de cualquier flujo de nodos)
    // =========================

    // 1) Primer mensaje del día -> saludo LARGO + (imagen si corresponde) + menú
    if (!$hasGreetedToday) {
        $this->updateSessionNode($waFrom, $mainId);

        $greetingLong = $this->greetingTextAB($profileName, 'B'); // Largo (Nandito)
        $messages = [];
        $messages[] = ['body' => $greetingLong];
        ...
        $this->updateGreetingMeta($waFrom, 'B', $sentMedia, true);

        $this->log($waFrom, 'IN', $text);
        $this->log($waFrom, 'OUT', $greetingLong . "
...
" . $menu);
    ...
    // 2) Ya saludó hoy -> si el usuario saluda ("Hola", etc), responder saludo CORTO + menú
    if ($this->isGreeting($cmd)) {
        $this->updateSessionNode($waFrom, $mainId);

        $greetingShort = $this->greetingTextAB($profileName, 'A'); // Corto
        $messages = [];
        $messages[] = ['body' => $greetingShort];
        $messages[] = ['body' => $menu];

        // Actualiza last_greeted_at (mismo día) y mantiene media_sent
        $this->updateGreetingMeta($waFrom, 'A', false, true);
