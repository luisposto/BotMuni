...
if ($mode !== 'HUMAN') {
    ...
    if ($shouldGreet || $shouldMedia) {
        ...
        // Menú al final: si hay imagen, la mandamos EN EL MISMO MENSAJE que el menú (caption),
        // así WhatsApp lo muestra como: imagen arriba + texto abajo, evitando reordenamientos visuales.
        if ($mediaUrl !== '') {
            $messages[] = ['body' => $menu, 'media_url' => $mediaUrl];
        } else {
            $messages[] = ['body' => $menu];
        }
