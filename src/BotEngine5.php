...
if ($mode !== 'HUMAN') {
    ...
    // Saludo corto: si ya saludó hoy pero el usuario envía un saludo (HOLA/BUENAS/etc)
    $shouldShortGreet = (!$shouldGreet) && $this->isGreeting($cmd) && ((int)($session['suppress_greeting'] ?? 0) === 0);
// Media solo la primera vez (ever) si hay media_url configurado en MAIN.
    ...
    $mainId = $this->getNodeIdByKey('MAIN');
    $shouldMedia = ($shouldGreet && ((int)($session['media_sent'] ?? 0) === 0));

    // Disparadores: si toca saludar hoy o si toca mandar media por 1ra vez
    if ($shouldGreet || $shouldShortGreet || $shouldMedia) {
        $this->updateSessionNode($waFrom, $mainId);

        $menu = $this->renderNode($mainId);
        $variant = $shouldShortGreet ? 'A' : 'B';
...
$messages = [];
        ...
        if ($shouldGreet || $shouldShortGreet) {
        ...
        // Menú al final: enviamos la imagen y el menú en mensajes separados (más robusto: el menú siempre se ve).
        if ($mediaUrl !== '') {
            // IMAGEN sola (sin body): evita que WhatsApp la trate como sticker con caption oculto
            $messages[] = ['media_url' => $mediaUrl];
        }

        // MENÚ siempre como texto aparte (garantiza visibilidad)
        $messages[] = ['body' => $menu];
