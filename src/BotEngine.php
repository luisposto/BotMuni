...
// Orden garantizado: saludo -> imagen -> menú
// Reglas:
// - Saluda 1 vez por día (según TZ de negocio)
// - La imagen (media_url del MAIN) se envía solo 1 vez en la vida del usuario (media_sent)
// - Si vuelve desde HUMANO (suppress_greeting=1), NO saluda
// =========================
if ($mode !== 'HUMAN') {
    // TZ configurable desde bot_settings(BUSINESS_HOURS) -> {"tz":"..."}
    ...
    $mainId = $this->getNodeIdByKey('MAIN');
    $menu = $this->renderNode($mainId);
    $mediaUrl = $this->getNodeMediaUrl($mainId);

    // Imagen: solo 1 vez ever (si hay media_url configurado en MAIN)
    $shouldMedia = ((int)($session['media_sent'] ?? 0) === 0) && ($mediaUrl !== '');

    // 1) Primer mensaje del día -> saludo (A/B) + (imagen si corresponde) + menú
    if ($this->shouldGreetToday($session, $cmd, $tz)) {
        $this->updateSessionNode($waFrom, $mainId);

        $variant = $this->pickGreetVariant($session); // A=corto, B=largo
        ...
        $sentMedia = false;
        if ($shouldMedia) {
            // Media sola (sin caption) para evitar que WhatsApp oculte el texto
            $messages[] = ['media_url' => $mediaUrl];
            $sentMedia = true;
        }

        $messages[] = ['body' => $menu];

        // Marca saludo de hoy + guarda variante (solo la 1ra vez) + marca media_sent si aplica
...
" . $menu);
    ...
    // 2) Ya saludó hoy -> si el usuario saluda, responder saludo CORTO + menú (sin imagen)
    // (No cambia la variante A/B guardada; solo contesta educadamente.)
    if ($this->isGreeting($cmd) && (int)($session['suppress_greeting'] ?? 0) === 0) {
        $this->updateSessionNode($waFrom, $mainId);

        $greetingShort = $this->greetingTextAB($profileName, 'A');
        $messages = [
            ['body' => $greetingShort],
            ['body' => $menu],
        ];

        // Actualiza last_greeted_at (mismo día) y baja suppress_greeting si estaba encendido
        $keepVariant = (string)($session['greet_variant'] ?? 'A');
        $this->updateGreetingMeta($waFrom, $keepVariant, false, true);

        $this->log($waFrom, 'IN', $text);
        $this->log($waFrom, 'OUT', $greetingShort . "
...
👉 Escribí MENU para volver.";
    ...
    private function ensureSessionMetaColumns(): void {
        static $done = false;
        if ($done) return;
        $done = true;
        ...
        try {
            $cols = [];
            $stmt = $pdo->prepare("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'user_sessions'");
            $stmt->execute();
            ...
            if (!isset($cols['last_greeted_at'])) {
                $alterParts[] = "ADD COLUMN last_greeted_at DATETIME NULL";
            }
            if (!isset($cols['greet_variant'])) {
                $alterParts[] = "ADD COLUMN greet_variant CHAR(1) NULL";
            }
            if (!isset($cols['media_sent'])) {
                $alterParts[] = "ADD COLUMN media_sent TINYINT(1) NOT NULL DEFAULT 0";
            }
            if (!isset($cols['suppress_greeting'])) {
                $alterParts[] = "ADD COLUMN suppress_greeting TINYINT(1) NOT NULL DEFAULT 0";
            }

            if (!empty($alterParts)) {
                $pdo->exec("ALTER TABLE user_sessions " . implode(", ", $alterParts));
            }
        } catch (Exception $e) {
            // Si no tenemos permisos de ALTER en hosting, seguimos sin romper el bot.
            // En ese caso, simplemente no se podrá usar saludo 1 vez por día / media única.
        }
    }
