...
if ($mode !== 'HUMAN') {
    ...
    if ($shouldGreet || $shouldMedia) {
        ...
        // Mejor compatibilidad WhatsApp/Twilio: enviar media + caption en el MISMO mensaje.
        // Si toca imagen, usamos como caption el saludo (si corresponde) y si no, mandamos el menú.
        if ($shouldMedia) {
            $media = $this->getNodeMediaUrl($mainId);
            if ($media !== '') {
                $caption = $shouldGreet ? $greeting : $menu;
                $messages[] = ['body' => $caption, 'media_url' => $media];
                $sentMedia = true;

                // Si ya mandamos el saludo como caption, no lo repetimos.
                if ($shouldGreet) {
                    $shouldGreet = false;
                }
            }
        ...
        // Menú siempre al final (si ya se mandó como caption de la imagen, igual lo reforzamos acá)
        $messages[] = ['body' => $menu];
