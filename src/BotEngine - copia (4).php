...
if ($mode !== 'HUMAN') {
    ...
    // Media solo la primera vez (si hay media_url configurado en MAIN)
    $mainId = $this->getNodeIdByKey('MAIN');
    $shouldMedia = ((int)($session['media_sent'] ?? 0) === 0) && ((bool)($session['is_new'] ?? false));
...
👉 Escribí MENU para volver.";
    ...
    private function isSameLocalDay(?string $lastGreetedAt, string $tz): bool {
        ...
        try {
            $d = new DateTime($lastGreetedAt, new DateTimeZone($tz));
    ...
    private function shouldGreetToday(array $session, string $cmd, string $tz): bool {
        ...
        // Siempre saludar si es sesión nueva (primer contacto), salvo suppress
        if ((bool)($session['is_new'] ?? false)) return true;

        // Disparadores "naturales"
        if ($cmd === '' || $this->isGreeting($cmd)) return true;

        // Si querés que MENU también salude el 1er msg del día, agregalo acá.
        return false;
    }
    ...
    private function greetingTextAB(string $profileName, string $variant): string {
        ...
        // Largo
        return ($name === '')
            ? "👋 ¡Hola! Soy SanFer el asistente de Tránsito de San Fernando.\nEstoy acá para ayudarte."
