...
// Si está en modo HUMANO, permitir volver al bot con MENU/0/etc
        if ($mode === 'HUMAN') {
        
            // Comandos para volver al bot
            if ($cmd === 'MENU' || $cmd === 'MENU.' || $cmd === 'MENÚ' || $cmd === '0' || $cmd === 'INICIO' || $cmd === 'BOT' || $cmd === 'VOLVER') {
        ...
        if ($text === '' || $cmd === 'MENU' || $cmd === 'MENU.' || $cmd === 'MENÚ' || $cmd === 'MENÚ' || $cmd === '0' || $cmd === 'INICIO' || $cmd === 'VOLVER') {
            $currentNodeId = $this->getNodeIdByKey('MAIN');
            $this->updateSessionNode($waFrom, $currentNodeId);
            $menu = $this->renderNode($currentNodeId);
            $this->log($waFrom, 'IN', $text);
...
private function renderNodeTextOnly(int $nodeId): string {
    ...
    // Opcional: agregás una guía mínima de navegación
    return trim($msg) . "\n\n👉 Escribí MENU para volver.";
}
